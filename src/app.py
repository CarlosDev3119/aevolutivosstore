from config.databaseConfig import cargar_datos
import pandas as pd
...
from functools import partial  # Para pasar df a la función de evaluación
...
def evaluar_individuo(individuo, df):
...
def algoritmo_evolutivo(df, pop_size=50, ngen=20, cxpb=0.5, mutpb=0.2):
    # Registrar la función de evaluación con el DataFrame
    toolbox.register("evaluate", partial(evaluar_individuo, df=df))
...
def main():
    ...
    # Convertir la columna 'tamaño' a valores numéricos
    ...
    pop, logbook = algoritmo_evolutivo(df,pop_size=100, ngen=50)
